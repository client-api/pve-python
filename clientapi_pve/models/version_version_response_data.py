...
class VersionVersionResponseData(BaseModel):
    """
    VersionVersionResponseData
    """ # noqa: E501

    console: Optional[PveConsoleEnum] = Field(default=None, description="The default console viewer to use.")

    release: StrictStr = Field(description="The current Proxmox VE point release in `x.y` format.")

    repoid: Annotated[str, Field(strict=True)] = Field(description="The short git revision from which this version was build.")
    ...
    def repoid_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not isinstance(value, str):
            value = str(value)

        if not re.match(r"[0-9a-fA-F]{8,64}", value):
            raise ValueError(r"must validate the regular expression /[0-9a-fA-F]{8,64}/")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of VersionVersionResponseData from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of VersionVersionResponseData from a dict"""
        if obj is None:
            return None
