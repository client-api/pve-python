...
class ClusterBackupGetBackupResponseDataInnerFleecing(BaseModel):
    """
    Options for backup fleecing (VM only).
    """ # noqa: E501

    enabled: Optional[PveBoolean] = Field(default=None, description="Enable backup fleecing. Cache backup data from blocks where new guest writes happen on specified storage instead of copying them directly to the backup target. This can help guest IO performance and even prevent hangs, at the cost of requiring more storage space.")

    storage: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Use this storage to storage fleecing images. For efficient space usage, it's best to use a local storage that supports discard and either thin provisioning or sparse files.")

    __properties: ClassVar[List[str]] = ["enabled", "storage"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ClusterBackupGetBackupResponseDataInnerFleecing from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ClusterBackupGetBackupResponseDataInnerFleecing from a dict"""
        if obj is None:
            return None
